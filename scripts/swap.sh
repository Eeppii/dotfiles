...
# This is a script that changes the focus from one screen to other when being executed.
...
hardcoded_width=1440
hardcoded_height=1413
...
(( $X >= $hardcoded_width )) && focustoscreen=0 || focustoscreen=1;

# get coordinates of the middle of the screen we want to switch
# searchx=$[ ($WIDTH / 2) + $focustoscreen * $WIDTH ]
# searchy=$[ $HEIGHT / 2 ]
searchx=$[ ($hardcoded_width / 2) + $focustoscreen * $hardcoded_width ]
searchy=$[ $hardcoded_height / 2 ]
